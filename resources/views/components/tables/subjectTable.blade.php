...
<div class="container">
    ...
    <div class="modal fade" id="deleteModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Xoá môn học</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
    ...
    const App = new Vue({
        ...
        methods: {
            checkSubject(subject){
                if (!subject) {
                    this.errors.push('Không được để trống tên môn học.');
                    return false;
                }

                if (!this.validSubject(subject)) {
                    this.errors.push('Định dạng tên môn học không chính xác.');
                    return false;
                }
                return true;
            },
            ...
            checkNumberOfCredits(number){
                if (!number) {
                    this.errors.push('Không được để trống số tín chỉ.');
                    return false;
                }

                if (!this.validNumber(number)) {
                    this.errors.push('Định dạng số tín chỉ không chính xác.');
                    return false;
                }
                return true;
            },
            validSubject: function (subject) {
                ...
                return regexIdentifier.test(subject.replace(/\s/g, ''));
                },

            validSubjectCode: function (code) {
                var fnNameRegex = /[ !@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/;
                return fnNameRegex.test(code);
                },    
            ...
            editSubject(subjectId) {
                this.errors = [];
                let subject = this.editingSubject.name;
                let subjectCode = this.editingSubject.subject_code
                let numberOfCredits = this.editingSubject.number_of_credits;
                if(!this.checkSubject(subject) | !this.checkSubjectCode(subjectCode) | !this.checkNumberOfCredits(numberOfCredits)){
                    console.log("fail");
                }else{
